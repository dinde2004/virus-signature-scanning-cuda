    ...
    std::vector<MatchResult> &matches)
{
    ...
    // Calculate total lengths and offsets for samples and signatures
    ...
    // Allocate char arrays for concatenated samples and signatures
    ...
    // Copy data from vectors into concatenated arrays
    for (int i = 0; i < num_samples; i++)
    {
        memcpy(&h_sample_sequences[h_sample_offsets[i]], samples[i].seq.c_str(), h_sample_lengths[i]);
        memcpy(&h_sample_qualities[h_sample_offsets[i]], samples[i].qual.c_str(), h_sample_lengths[i]);
    }
    for (int i = 0; i < num_signatures; i++)
    {
        memcpy(&h_signature_sequences[h_signature_offsets[i]], signatures[i].seq.c_str(), h_signature_lengths[i]);
    }
    ...
    // Copy data to device
    cudaMemcpy(d_sample_sequences, h_sample_sequences, total_sample_length * sizeof(char), cudaMemcpyHostToDevice);
    cudaMemcpy(d_sample_qualities, h_sample_qualities, total_sample_length * sizeof(char), cudaMemcpyHostToDevice);
    cudaMemcpy(d_sample_offsets, h_sample_offsets, num_samples * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_sample_lengths, h_sample_lengths, num_samples * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_signature_sequences, h_signature_sequences, total_signature_length * sizeof(char), cudaMemcpyHostToDevice);
    cudaMemcpy(d_signature_offsets, h_signature_offsets, num_signatures * sizeof(int), cudaMemcpyHostToDevice);
